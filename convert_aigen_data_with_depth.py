...
def convert(input_dir: str, output_path: str):
    ...
    with h5py.File(files[0], 'r') as hf:
        ...
        depth_shape = hf['depth'].shape[1:] if has_depth else None

    print(f"Image shape: {img_shape}, Action dim: {action_dim}, Proprio dim: {proprio_dim}, Depth: {has_depth}")
    ...
    with h5py.File(output_path, 'w') as out:
        ...
        if has_depth:
            depth_ds = out.create_dataset(
                'depth', shape=(total_frames, *depth_shape), dtype=np.uint8,
                chunks=(min(256, total_frames), *depth_shape)
            )
        ...
        for i, f in enumerate(files):
            with h5py.File(f, 'r') as hf:
                ...
                if has_depth and 'depth' in hf:
                    depth_ds[offset:offset + n] = hf['depth'][:]
